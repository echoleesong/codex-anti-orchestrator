...
页面只监听 `http://127.0.0.1:<port>`，每三秒刷新，展示任务状态流转、PR 链接、测试与 CI 观察结果、Codex 人工核验清单、Anti 的 localhost 验证证据、经过脱敏的事件流，以及每一步调用前持久化保存的 AI 提示词审计记录（Anti 初始开发、Codex 审查、Anti 审查修复、Anti 本机核验）。提示词卡片使用原生 `<details>`/`<summary>` 控件默认折叠展示，并在自动刷新期间保持用户的展开/折叠交互状态。严格执行路径脱敏、密钥遮蔽、环境变量抑制与长度截断，避免敏感信息泄露。
