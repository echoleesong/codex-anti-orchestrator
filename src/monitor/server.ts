...
import type { TaskEvent, TaskPromptAudit, TaskRecord } from '../types.js';
import { defaultExecutor, redactSecrets } from '../utils/exec.js';

const LOCALHOST = '127.0.0.1';
const MAX_TASKS = 100;
const MAX_EVENTS_PER_TASK = 100;
const MAX_PROMPT_AUDITS_PER_TASK = 30;
const MAX_PROMPT_AUDIT_BODY_LENGTH = 10_000;
...
function safePromptAudits(
  audits: TaskPromptAudit[] | undefined,
  context?: { worktreePath?: string; targetRepoPath?: string }
): Array<{
  timestamp: string;
  actor: string;
  stage: string;
  ...
  return audits.slice(-MAX_PROMPT_AUDITS_PER_TASK).map((audit) => ({
    timestamp: audit.timestamp,
    actor: safeText(audit.actor, 50) || 'UNKNOWN',
    stage: safeText(audit.stage, 50) || 'UNKNOWN',
    title: safeText(audit.title, 100) || '提示词记录',
    body: sanitizePromptAuditText(audit.body, {
      worktreePath: context?.worktreePath,
      targetRepoPath: context?.targetRepoPath,
      maxLength: MAX_PROMPT_AUDIT_BODY_LENGTH,
    }),
  }));
}
...
async function taskView(task: TaskRecord, detail = false): Promise<Record<string, unknown>> {
  const basic = {
    ...
    promptAuditCount: (task.promptAudits || []).length,
  };
  ...
  return {
    ...
    promptAudits: safePromptAudits(task.promptAudits, {
...
const promptAuditPanel=(audits,openIds)=>{if(!audits||!audits.length)return '<div class="empty">暂无提示词记录</div>';return audits.map((audit,index)=>{const auditId=String(index);const isOpen=openIds&&openIds.has(auditId);return '<details class="prompt-audit" data-audit-id="'+auditId+'"'+(isOpen?' open':'')+'><summary class="prompt-summary"><div class="prompt-summary-main"><span class="prompt-actor">'+esc(label(audit.actor))+'</span><span class="prompt-title">'+esc(audit.title)+'</span><span class="status '+esc(audit.stage)+'">'+esc(label(audit.stage))+'</span></div><div class="prompt-summary-meta"><time>'+esc(stamp(audit.timestamp))+'</time></div></summary><div class="prompt-body"><div class="prompt-body-meta"><span>执行者：<strong>'+esc(label(audit.actor))+'</strong></span><span>阶段：<strong>'+esc(label(audit.stage))+'</strong></span><span>时间：<time>'+esc(stamp(audit.timestamp))+'</time></span></div><pre class="prompt-pre">'+esc(audit.body)+'</pre></div></details>';}).join('');};
