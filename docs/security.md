...
### 2.12 AI Prompt Auditing & Sanitization Boundary

- **Step-by-Step AI Prompt Recording**: The exact final sanitized prompt dispatched to external AI models is recorded before execution for: Anti initial development (`Anti 初始开发`), Codex review (`Codex 审查`), Anti review fix (`Anti 审查修复`), and Anti local verification (`Anti 本机核验`).
- **Interruption Resilience**: Each prompt audit record is saved to disk atomically before invoking external child processes. If a task terminates unexpectedly or crashes mid-execution, previously recorded steps are guaranteed to be preserved.
- **Strict Path & Secret Redaction**: Stored and API-returned prompts are scrubbed via `sanitizePromptAuditText` and `redactSecrets`. Absolute paths across platforms (worktree path, target repo path, state directory, home directory, `/Users/...`, `/home/...`, `/private/...`, `/var/...`, `/tmp/...`, `C:\...`) are replaced with relative tokens or `[PATH]`.
- **Environment Variable Suppression**: Process environment variables and `KEY=VALUE` assignments in prompts are scrubbed (`[REDACTED_ENV]`).
- **Bounded Retention & Length**: Records per task and character length per prompt are capped to prevent memory exhaustion and respect the 512 KB monitor response limit.
- **Native Accessible Presentation**: Displayed in the localhost-only monitor UI using native accessible `<details>` / `<summary>` controls, collapsed by default, with expansion state preserved across auto-refreshes.
...
| **Sensitive path, secret, or env leak in prompt audits** | Exposure of local system paths or credentials | Pre-persistence and pre-response sanitization with multi-pattern scrubbing, absolute path stripping, env suppression, and bounded payloads. |
