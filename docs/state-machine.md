...
stateDiagram-v2
    [*] --> IDLE
    IDLE --> INITIALIZING: Task Received (allowed directory confirmed)
    ...
    REVIEW_EVALUATING --> AGY_VALIDATING: Tests Pass, CI Passes & Codex Review Clean
    REVIEW_EVALUATING --> AGY_FIXING: Issues Found & Cycles < Max
    REVIEW_EVALUATING --> NEEDS_USER_DECISION: Issues Found & Cycles == Max

    AGY_VALIDATING --> AWAITING_HUMAN_APPROVAL: Localhost app started & checklist verified
    AGY_VALIDATING --> NEEDS_USER_DECISION: No verifiable local runtime evidence
...
| :------------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `IDLE`                    | Waiting for incoming task request from Codex App MCP.                                                                                          | No active child processes; no temporary worktrees allocated. Before a first task on a machine, the user must explicitly confirm the allowed repository directory.                                                                                                                                                                                 |
| `INITIALIZING`            | Verifying tool readiness (`doctor`), confirmed repository directory, target branch, and task payload.                                          | Read-only check; fails fast if the allowed directory is absent or invalid, or if required tools, credentials, or remote origins are missing/invalid.                                                                                                                                                                                              |
...
| `CODEX_REVIEWING`         | Running deterministic preflight and, only when green, invoking quota-bounded native Codex review.                                              | `git diff --check` and available allowlisted package scripts run before model budget reservation. Real Codex calls are durably capped, duplicate review identities are cached, and only a prior `APPROVE` ancestor may become an incremental base.                                                                                                |
| `REVIEW_EVALUATING`       | Evaluating review findings, deterministic test evidence, and GitHub CI.                                                                        | A passing default `test` result from the immediately preceding preflight is reused instead of rerunning the same suite. Custom test runners or projects without a standard `test` script still use the normal fallback runner. Pending CI is polled with bounded attempts; terminal failure/unavailability fails closed to `NEEDS_USER_DECISION`. |
| `AGY_VALIDATING`          | Anti starts the changed application locally and executes Codex's handoff checklist.                                                            | Anti must return structured evidence containing the local command, a `127.0.0.1` or `localhost` URL, completed observable checks, and a summary. It must stop the server before returning and must not modify the worktree. Missing, malformed, non-local, or failed evidence enters `NEEDS_USER_DECISION`.                                       |
...
| `AWAITING_HUMAN_APPROVAL` | PR is 100% clean and green (all automated tests pass, PR CI checks pass, Codex review has **no blocking issues**, and live validation passed). | **Strict invariant**: Only reachable directly from `AGY_VALIDATING` on clean pass (`reviewClean === true`, `testsPass === true`, `ciPassing === true`, `liveVerificationPassed === true`, and structured `ciProof` with `allPassing === true`). Automated execution halts; worktree is preserved in state directory; awaits human manual merge.   |
...
- **Maximum Review-Fix Cycles (`MAX_REVIEW_CYCLES`)**: Default: `3`. This bounds state-machine fix iterations and is separate from model quota.
- **Hard Codex Model-Call Budget**: At most `3` real Codex review invocations per task worktree are reserved durably. Deterministic preflight failures and cache hits consume no additional model call. A duplicate immutable review identity is never automatically reviewed twice.
- **Review Scope Invariant**: Only a previously `APPROVE` HEAD that is an ancestor of the current HEAD may become an incremental review base. After `CHANGES_REQUIRED`, fixes receive a full re-review from the original base so unresolved old findings remain visible.
- **Preflight Evidence Reuse**: If the current review's deterministic preflight actually ran and passed the project's standard `test` script, `REVIEW_EVALUATING` reuses that evidence. Custom test runners and missing test scripts still execute the fallback test path.
- **Strict Invariant for `AWAITING_HUMAN_APPROVAL`**:
  - `AWAITING_HUMAN_APPROVAL` is **strictly unreachable** from `NEEDS_USER_DECISION`.
  - It can **only** be reached directly from `AGY_VALIDATING` when **all** conditions are explicitly true:
    ...
    5.  **`liveVerificationPassed === true`**: Anti has actually started the project at a loopback URL and returned structured evidence for Codex's human-verification checklist. No generic claim or external URL is accepted.
