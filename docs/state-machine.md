...
| `AWAITING_HUMAN_APPROVAL` | PR is 100% clean and green (all automated tests pass and Codex review has **no blocking issues**). | **Strict invariant**: Only reachable directly from `REVIEW_EVALUATING` on clean pass (`reviewClean === true` and `testsPass === true`). Automated execution halts; worktree is preserved in state directory; awaits human manual merge.     |
